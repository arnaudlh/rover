...
function get_list_of_task() {
    ...
    find "$ci_task_dir" -maxdepth 1 -type f \( -name "*.yml" -o -name "*.yaml" \) | LC_ALL=C sort | tr '\n' ' ' | sed 's/ $//'
}

function get_task_parameters_json() {
    local config_file=$1
    yq eval '.parameters' "$config_file" -o=json
}

function get_task_name() {
    local config_file=$1
    basename "$config_file" .yml
}
...
function format_task_parameters {
  ...
  echo "$json" | jq -r 'to_entries | .[] | if .value.prefix then "--" else "" end + .key + "=" + .value.value' 2>/dev/null || {
    echo "$json" | jq -e . >&2
    return 1
  }
}
